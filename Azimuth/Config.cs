...
namespace Azimuth
{
	public class Config
	{
		...
		public static VALUE? Get<VALUE>(string _category, string _key)
		{
			...
			{
				//Attempt to get the vector2 value fron the config, then try to change it to the correct type
				return (VALUE) Convert.ChangeType(instance.vector2s[_category][_key], valueType);
			}

			if(valueType == typeof(Vector3))
			{
				//Attempt to get the vector3 value fron the config, then try to change it to the correct type
				return (VALUE) Convert.ChangeType(instance.vector3s[_category][_key], valueType);
			}

			if(valueType == typeof(Color))
			{
				//Attempt to get the Colors value fron the config, then try to change it to the correct type
				return (VALUE) Convert.ChangeType(instance.colors[_category][_key], valueType);
			}

			if(valueType == typeof(int))
			{
				//Attempt to get the int value fron the config, then try to change it to the correct type
				return (VALUE) Convert.ChangeType(instance.ints[_category][_key], valueType);
			}

			if(valueType == typeof(float))
			{
				//Attempt to get the float value fron the config, then try to change it to the correct type
				return (VALUE) Convert.ChangeType(instance.floats[_category][_key], valueType);
			}

			if(valueType == typeof(bool))
			{
				//Attempt to get the bool value fron the config, then try to change it to the correct type
				return (VALUE) Convert.ChangeType(instance.bools[_category][_key], valueType);
			}

			if(valueType == typeof(string))
			{
				//Attempt to get the string value fron the config, then try to change it to the correct type
		...
		private void Load()
		{
			...
			//if the directory doesnt exist, creat it and return
			...
			using(StreamReader reader = new StreamReader(FilePath))
			{
				...
				//assign line to the nect line in the file, then check if null
				//line will only be null if we reached the end of the file
				//empty lines arent considered null
				while((line = reader.ReadLine()) != null)
				{
					currentCatagory = ProcessLine(line, currentCatagory);
				}
			}
		}
		...
		{
			return !(_val.Any(char.IsLetter) || !_val.Any(_c => _c == '.'));
		}
